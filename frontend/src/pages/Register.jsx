...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!err?.response) {
        setApiError('Unable to connect to backend server. Please make sure the backend is running on http://localhost:8000.');
      } else if (err?.response?.status === 409) {
        setApiError('This email address is already registered. Please login instead.');
      } else {
